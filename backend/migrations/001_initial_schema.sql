-- 1. Raw Data Tables (Previously in fetch_and_store_*.py)
...
CREATE TABLE IF NOT EXISTS editor_counts (
    timestamp DATETIME,
    editor_count INT,
    project VARCHAR(255),
    PRIMARY KEY (timestamp, project)
...
-- 2. Alert/Peak Tables (Previously in *_alerts.py)
...
CREATE TABLE IF NOT EXISTS editor_alerts (
    project VARCHAR(255),
    timestamp DATETIME,
    editor_count INT,
    rolling_mean FLOAT,
...
-- 3. Annotation Layer (The new feature)
...
-- 4. User Subscription System for Peak Notifications
