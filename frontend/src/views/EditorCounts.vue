<template>
  ...
  <div class="min-h-screen bg-gray-50 font-sans text-[#202122]">
    ...
    <div class="flex pt-16">
      ...
      -->
      <SidebarFilters @fetch-data="fetchEditorData" />
      ...
      >
        <div class="max-w-7xl mx-auto min-h-[calc(100vh-20rem)]">
          ...
          <div v-if="loading" class="flex flex-col items-center justify-center h-[60vh] animate-fade-in">
            ...
            <p class="text-gray-500">Retrieving editor activity and statistics...</p>
          </div>
          ...
          <div v-else-if="editorData" class="space-y-6 animate-fade-in">
            <!-- 
              Passing context to the chart 
              This ensures the chart title says "English Wikipedia Editors" instead of just "Activity"
            -->
            <ActivityChart 
              :chart-data="editorData.chartData" 
          ...
          <div v-else class="flex flex-col items-center justify-center h-[60vh] text-center">
            <div class="codex-card p-12 max-w-2xl border-dashed">
              ...
              <h2 class="text-2xl font-bold text-gray-900 mb-3">Editor Activity Alerts</h2>
              <p class="text-lg text-gray-500 max-w-md mx-auto leading-relaxed">
                Select a project (e.g., Wikipedia) and a language from the sidebar to visualize editor trends and anomalies.
              </p>
            </div>
          </div>
...
const fetchEditorData = async (filters) => {
  ...
  try {
    ...
    const response = await axios.get(
      `${import.meta.env.VITE_BACKEND_URL}/api/editor-activity-data`,
      { params }
    );

    editorData.value = response.data;
  } catch (err) {
